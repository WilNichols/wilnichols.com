/* Location derivations, shared by the directory data and the /test/location
   fixture so the two cannot drift. */
import { DateTime } from "luxon";

function coordsFrom(data) {
  /* `location` is what Obsidian's Maps plugin reads, so it wins. `coordinates`
     is an older alias. Both take "lat, lng" or [lat, lng]. */
...
/* Time is optional. YAML does the work: a bare date parses to a Date, `T14:30`
   stays a String. */
export function visitedDisplay(visited) {
  ...
  return list
    .map((v) => {
      ...
      return {
        /* Local wall-clock time, so `Z` would lie. */
